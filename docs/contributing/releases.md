...
These are the single source of truth for each line's supported minors, JDK, and dependency pins:
...
| `publish` | Gathers all ZIPs, writes `SHA256SUMS` and release notes, attests provenance, and runs `gh release create v<version> --target <sha>` (which creates the tag atomically). Runs in the `release` environment. |
...
Example assets for `v0.1.0`:

```text
opensearch-aosc-0.1.0-os2.15.zip
opensearch-aosc-0.1.0-os2.17.zip
opensearch-aosc-0.1.0-os2.19.zip
opensearch-aosc-0.1.0-os3.1.zip
opensearch-aosc-0.1.0-os3.3.zip
opensearch-aosc-0.1.0-os3.5.zip
...
`versions.json` on `gh-pages` is the authoritative list of published versions; the deploy script maintains it, so no version paths are hardcoded here or in the site config.
